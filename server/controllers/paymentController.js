...
exports.createPayOSPayment = async (req, res) => {
  try {
    ...
    // Validate bắt buộc
    if (!amount || !description || !buyerName || !buyerEmail) {
      return res.status(400).json({
        success: false,
        message: 'Thiếu thông tin bắt buộc (amount, description, buyerName, buyerEmail)'
      });
    }

    // Tạo orderCode ngẫu nhiên (dùng timestamp hoặc random 6 chữ số)
    const orderCode = Math.floor(100000 + Math.random() * 900000); // Ví dụ: 623498
