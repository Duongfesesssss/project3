<template>
    <Dialog
      ...
      :header="`${
        props.nhaCungCap?._id === null || props.nhaCungCap?._id === undefined
          ? 'Thêm mới '
          : 'Cập nhật '
      } nhà cung cấp`"
      :modal="true"
  ...
  const props = defineProps({
    ...
    nhaCungCap: {
      type: Object as () => NhaCungCapModel,
    },
  });
  ...
  const schema = yup.object({
    name: yup.string().required('Tên nhà cung cấp là bắt buộc').max(256, 'Tối đa 256 ký tự'),
    ...
    ConfirmDialog.showConfirmDialog(
      ...
      `${
        NhaCungCapDTO._id
          ...
          : 'Bạn có chắc muốn thêm thông tin nhà cung cấp này?'
      }`,
      'Xác nhận',
      'pi pi-question-circle',
      () => {
        if (NhaCungCapDTO._id != null) {
          NhaCungCapService.update(NhaCungCapDTO as NhaCungCapModel)
            .then((response) => {
              if (response?.status == EnumStatus.OK) {
                toast.add({
                  severity: 'success',
                  summary: 'Thành công',
                  detail: 'Cập nhật thông tin nhà cung cấp thành công!',
                  life: 3000,
                });
                ...
                toast.add({
                  severity: 'error',
                  summary: 'Thất bại',
                  detail: 'Cập nhật thông tin nhà cung cấp không thành công!',
                  life: 3000,
                });
          ...
          NhaCungCapService.insert(NhaCungCapDTO as NhaCungCapModel)
            .then((response) => {
              if (response?.status == EnumStatus.OK) {
                toast.add({
                  severity: 'success',
                  summary: 'Thành công',
                  detail: 'Thêm mới thông tin nhà cung cấp thành công!',
                  life: 3000,
                });
                ...
                toast.add({
                  severity: 'error',
                  summary: 'Thất bại',
                  detail: 'Thêm mới thông tin nhà cung cấp không thành công!',
                  life: 3000,
                });
